// Web-sized copies generated by `npm run optimize:images`.
import portfolioImg from "/src/assets/optimized/portfolio.webp";
import guesspkmnImg from "/src/assets/optimized/guesspkmn.webp";
import codepieceImg from "/src/assets/optimized/codepiece.webp";
import pokemonClassifierImg from "/src/assets/optimized/pkmn-class.webp";

export const projects = [
  {
    title: "Portfolio Website",
    date: "August 2025",
    description: "Designed and developed a personal portfolio website to showcase my projects and skills using modern web technologies.",
    technologies: ["React.js", "HTML", "CSS", "JavaScript", "Cloudflare"],
    source: "https://github.com/alyanasmat13/Website-Portfolio",
    image: portfolioImg
  },
  {
    title: "GuessPKMN",
    date: "September 2025",
    description: "Built interactive web application that challenges users to identify Pokemon through gamified quiz mechanics with point-based scoring system and randomized content generation.",
    technologies: ["React", "HTML", "Tailwind CSS", "TypeScript", "PokéAPI", "Firebase", "Vercel", "Render"],
    source: "https://github.com/alyanasmat13/GuessPKMN",
    image: guesspkmnImg,
    link: "https://guesspkmn.vercel.app/"
  },
  {
    title: "CodePiece",
    date: "May 2026",
    description: "Built a real-time multiplayer word game supporting concurrent players across separate devices, engineered around a server-authoritative architecture to act as the single source of truth for game state.",
    technologies: ["React", "Node/Express", "Tailwind CSS", "TypeScript", "Socket.IO", "Render"],
    source: "https://github.com/alyanasmat13/code-piece",
    image: codepieceImg,
    link: "https://code-piece.onrender.com/"
  },
  {
    title: "Pokemon Classifier",
    date: "July 2026",
    description: "Fine-tuned a DistilBERT transformer to classify Pokemon descriptions into 151 species, achieving 79% accuracy. Built an automated data pipeline to gather 6,500+ training examples and deployed the model as a real-time FastAPI endpoint.",
    technologies: ["Python", "NumPy", "Pandas", "FastAPI", "Hugging Face"],
    source: "https://github.com/alyanasmat13/pokemon-classifier",
    image: pokemonClassifierImg,
  }

]
